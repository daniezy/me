...
def set_it_on_fire(input_string="very naughty boy") -> str:
    ...
    """
    return f"🔥{'🔥'.join(list(input_string.upper()))}🔥"
...
def make_filler_text_dictionary() -> Dict:
    ...
    for word_length in range(3, 8):
        url = f"{base_url}{word_length}"
        r = requests.get(url)
        words = r.text.split("\n")
        wd[word_length] = words[:4]
...
def random_filler_text(number_of_words=200) -> str:
    ...
    for _ in range(number_of_words):
        random_length = random.choice(list(wd.keys()))
        random_word = random.choice(wd[random_length])
        words.append(random_word)
    return " ".join(words)
    result = random_filler_text(200)
    print(result)
    return " ".join(words)
